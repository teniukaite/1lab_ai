...
def buildDecisionTree(data, columns, currentDepth=0, minSampleLeaf=2, maxDepth=1000, nEstimators=None): # sukuria decision tree
    if checkPurity(data) or len(data) < minSampleLeaf or currentDepth == maxDepth: # jei duomenys yra unikalus arba maziau nei minSampleLeaf arba pasiektas maxDepth
        ...
        potentialSplits = get_potential_splits(data, random_subspace=nEstimators) # paimam galimus splitus
